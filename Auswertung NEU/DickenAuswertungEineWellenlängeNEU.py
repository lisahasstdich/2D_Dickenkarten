...
sample = 'M17'
used_alpha ='dünn'
savepath = r'C:\Users\Lisa\Desktop\DickenkartenSVG\dünn\\'
...
#file = r'C:/Users/Lisa/Desktop/Uni/Bachelorarbeit/Oktober/Absorptionskoeffizienten/Alpha_E5_dünn.txt'
unten = 10
oben = 395  #Indices bei denen abgeschnitten werden soll
links = 10
rechts = 70
wavelength_evaluate = 590 #Wellenlänge, bei der ausgewertet wird
plot_show_default = True
plot_show_zugeschnitten = False
plot_profiltiefe_y = False
plot_profiltiefe_x = False
plot_einzelprofil_y = False
plt.rcParams.update({
...
class Absorption:
    ...
    def intensities(self,h5file = None,interpolation = None):
        ...
        wavelength_min = np.min(g['spectrum/wavelength']) 
        wavelength_max = np.max(g['spectrum/wavelength'])
        ...
        def Int0(self, h5file = None,scan_axis = None,
              ...
              intensityRef = g['spectrum/intensity'][:,index_wavelength]
        ...
        def Int1(self, h5file = None, scan_axis =None,
              ...
              # Initialize data for intensityRef
              timestamps = g['spectrum/timestamp'][:]
              intensity = g['spectrum/intensity'][:,index_wavelength]
        ...
        Int0=Int0( self )
        Int1, scan_ax, move_ax=Int1(self)
       ...
       # alpha_error =np.array(alpha_errors)
        ...
        print('log=', np.log(int_ratio[200,30]))
        ...
        if plot_show_zugeschnitten ==True:
            # links_index = np.searchsorted(move_ax, 0.9)
            # rechts_index = np.searchsorted(move_ax, 19.2)
            # oben_index = np.searchsorted(scan_ax, 12.51)
            # unten_index = np.searchsorted(scan_ax, 7.5)
            # move_ax=move_ax[links_index:rechts_index]
            # scan_ax=scan_ax[unten_index:oben_index]
            # dicke = dicke[unten_index:oben_index, links_index:rechts_index]
            ...
            ax.set_xticks(np.arange(2, 21, 2.5))
            ax.yaxis.set_major_locator(MultipleLocator(1))
            plt.imshow(dicke, cmap=plt.cm.get_cmap('jet', 30), aspect='auto',
                       interpolation=interpolation,extent=[np.min(move_ax), np.max(move_ax), np.min(scan_ax), np.max(scan_ax)],
                       origin='lower')
            plt.colorbar()
            plt.clim(30,60)
           # fig.savefig(savepath + f'{sample}_{used_alpha}alpha.svg', format='svg')
            plt.show() 
            plt.close()
            if plot_einzelprofil_y == True:
                ...
                ax.legend(title='y (mm)', loc='best', ncols=3)
                ax.grid(True)
        ...
        if plot_profiltiefe_y == True:
           ...
           # plt.savefig(savepath + f'{sample}_Profil_y_gedickt.png', bbox_inches='tight', dpi=300)
            np.savetxt(savepath+f'{sample}_Profil_y_gemittelt_{used_alpha}.txt', np.column_stack([move_ax,mean_dicke_neu_y]), header='x, mean_dicke_y', footer=f'gedickt über y-Werte: {desired_y_values}', delimiter=' ')
            plt.show()           
            plt.close()
