...
savepath = r'C:\Users\Lisa\Desktop\Uni\Bachelorarbeit\Juli\Dickenkarten\\'
...
class Absorption:
    ...
    def intensities(self,h5file = None, interpolation = None):
       ...
       interpolation = interpolation or f'{variablesRef.interpolation}'
       g = h5py.File(h5file, "r")
       wavelength_min = 570#Wellenlängen, die man ausgewertet haben möchte
       wavelength_max = 620
      ...
      # alpha_errors = df.iloc[:,2]
       ...
       def Int0(self, h5file = None,
             ...
             wavelength_array = g[f"{variablesRef.data_group[1]}/wavelength"]
             ...
             wavelength_values = (g[f"{variablesRef.data_group[1]}/wavelength"][:])
             ...
             intensityRef = g[f'{variablesRef.data_group[1]}/intensity'][:,indices_wave]
       ...
       def Int1(self, h5file = None,
             ...
             wavelength_values = (g[f"{variables.data_group[1]}/wavelength"][:])
             ...
             intensity = g[f'{variables.data_group[1]}/intensity'][:,indices_wave]
       ...
       Int0=Int0(r'C:\Users\Lisa\Desktop\Uni\Bachelorarbeit\mai\Filme\2024-05-24\11-10-14\data_11-10-14.h5')
       Int1, scan_ax, move_ax=Int1(r'C:\Users\Lisa\Desktop\Uni\Bachelorarbeit\mai\Filme\2024-05-24\11-15-52\data_11-15-52.h5')
       int_ratio = Int0/Int1
       ...
       if calculate_thickness_difference == True:
           ...
           if save_difference_plot == True:
               fig.savefig(r'C:\Users\Lisa\Desktop\DickenkartenSVG\Fehlerkarten\M36_fehler_570620_dünn.svg', format='svg')
           plt.show()
           plt.close()
       if plot_spektrum == True:
             ...
             alpha_spek_1 = np.log10(Int0[y_mitte_index, x_1_index,:]/Int1[y_mitte_index, x_1_index, :])
             plt.plot(wavelengths_array[index_min_1:index_max_1], alpha_spek_1, color='red', linewidth =2, linestyle='-')
             plt.xlabel('wavelength (nm)')
             plt.ylabel('absorption')
             plt.title('Spectrum for x=5mm, y=11.25mm')
             plt.show()
             plt.close()  
             alpha_spek_2 = np.log10(Int0[y_mitte_index, x_2_index,:]/Int1[y_mitte_index, x_2_index, :])
             plt.plot(wavelengths_array[index_min_1:index_max_1], alpha_spek_2, color='red', linewidth =2, linestyle='-')
             plt.xlabel('wavelength (nm)')
             plt.ylabel('absorption')
             plt.title('Spectrum for x=11mm, y=11.25mm')
             plt.show()
             plt.close() 
             alpha_spek_3 = np.log10(Int0[y_mitte_index, x_3_index,:]/Int1[y_mitte_index, x_3_index, :])
             plt.plot(wavelengths_array[index_min_1:index_max_1], alpha_spek_3, color='red', linewidth =2, linestyle='-')
             plt.xlabel('wavelength (nm)')
             plt.ylabel('absorption')
             plt.title('Spectrum for x=17mm, y=11.25mm')
             plt.show()
             plt.close() 
             if save_spektrum_plot == True:
                 fig.savefig(savepath+ f'{sample}_spektrum.png', bbox_inches='tight', dpi=300)
       g.close()
